...
onMounted(() => {
  ...
  window.addEventListener('gravityly:pwa-offline-ready', handleOfflineReady);
  window.addEventListener('gravityly:pwa-update', handlePwaUpdate);
});
...
onUnmounted(() => {
  ...
  window.removeEventListener('gravityly:pwa-offline-ready', handleOfflineReady);
  window.removeEventListener('gravityly:pwa-update', handlePwaUpdate);
});
</script>
...
<template>
  <aside class="pwa-notices" aria-live="polite">
    ...
    <section v-if="showOfflineReady" class="pwa-notice" role="status">
      <span>O Gravityly está pronto para abrir offline.</span>
    ...
    <section v-if="hasUpdate" class="pwa-notice" role="status">
      <span>Uma nova versão do Gravityly está disponível.</span>
    ...
    <section v-if="canInstall" class="pwa-notice" role="status">
      <span>Instale o Gravityly para abrir como aplicativo.</span>
