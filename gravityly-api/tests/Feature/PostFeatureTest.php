...
it('creates an image-only post and returns a database-backed media URL', function () {
    $author = User::factory()->create();
    ...
    $response->assertJsonPath('post.image_url', '/media/posts/'.$post->id);
...
it('removes database-backed media when the post author deletes a post', function () {
    $author = User::factory()->create();
    ...
    $this->get('/media/posts/'.$post->id)->assertOk();
    ...
    $this->assertDatabaseMissing('posts', ['id' => $post->id]);
    $this->get('/media/posts/'.$post->id)->assertNotFound();
});
